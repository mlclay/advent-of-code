...
def main():
    ...
    for phase_setting in itertools.permutations(range(5), 5):
        for amplifier in range(5):
            ...
            program.input_queue.put(phase_setting[amplifier])
            if amplifier == 0:
                program.input_queue.put(0)
            else:
                program.input_queue.put(program.output_queue.get())

            program.run()

        phase_to_signal.append({
            'phase': phase_setting,
            'signal': program.output_queue.get()
        })

    print(max(phase_to_signal, key=lambda x: x['signal']))
